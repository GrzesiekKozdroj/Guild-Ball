let diceRoller = (Gamer, otherGamer, m1, m2 = m1, mode) => {
    let actualTac = 
        mode === 'kick' ? 
            m1.kick + (m1.bonusTime || m2.bonusTime ? 1 : 0) - otherGamer.squaddies.filter(el => distance(m1.posX, m1.posY, el.posX, el.posY) <= (el.meleeRadius + m1.baseRadius)).length - (m1.name !== m2.name && m2.inForest ? 1 : 0) - (m1.name !== m2.name && m1.inForest ? 1 : m1.inForest ? 1: 0):
